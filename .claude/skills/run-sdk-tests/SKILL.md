...
- Java 21+
- Podman or Docker
- `ko` — install with: `go install github.com/google/ko@latest`

## What the script does

1. Reads the suite version from `.github/workflows/integration.yaml` (single source of truth)
2. Builds the service image using **ko** (`KO_DOCKER_REPO=localhost/... ko build -B -L --tags=local`)
...
`ko` loads images via the Docker daemon. With Podman, set up socket compatibility:
